...
// # define  MAX_NAME_INF_ARRAY  80

const int MAX = 2000;

// const double MyLAT = 59.800095;		// так было при последнем успешном тестировании
...
// Status service flow
// volatile UINT status_service_thread;				// статус сервисного потока (если состояние == 0, то поток выполняется, 
													//							а	если состояние потока ==1, то поток выполнился)
// using namespace std;
...
/*
typedef  struct{
	char	dir_meo[MAX_NAME_INF_ARRAY];			// в этот массив заносится путь к директории, где содержатся МЭО-файлы
	int 	size_dir_meo;							// текущий размер содержимого массива dir_meo
	char	dir_mkeo[MAX_NAME_INF_ARRAY];	  		// в данный массив заносится путь к директории, где содержатся МКЭО-файлы
	int		size_dir_mkeo; 							// текущий размер содержимого массива dir_mkeo
	char	mod_io[MAX_NAME_INF_ARRAY];				// массив предназначен для хранения зарегистрированного глобального имени клиента - ПМ ИО (интерфейс оператора)
	int 	size_mod_io;							// текущий размер содержимого массива mod_io
	char	mod_bcu[MAX_NAME_INF_ARRAY];			// массив предназначен для хранения зарегистрированного глобального имени БКУ-клиента
	int 	size_mod_bcu;							// текущий размер содержимого массива mod_bcu
} config_data;
*/

/*
typedef struct{
	unsigned int id_inf_arj;						// ID (порядковый номер) информационного массива (МЭО или МКЭО)
	char name_inf_arj[MAX_NAME_INF_ARRAY];			// наименование информационного массива (МЭО или МКЭО)
	WORD size_array;								// размер содержимого в сроковой переменной (массиве)name_inf_arj
} id_name_inf_array;
*/
...
typedef struct{
	WORD type_inf_array;
	WORD size_array;
	id_name_inf_array tab_inf_arry[MAX_NAME_INF_ARRAY];
}tab_names_inf_array;
*/
...
//} TWgsPoint;
//------------------------------------------------------
// enum id_stutus_err { MEO_SERVER_OK, MEO_SERVER_ERR, MEO_SERVER_BAD_CMD, MEO_CONTAINS_NO_DATA, NOT_TRUE_GIVEN_NAME_OF_THE_INF_FILE };

// enum type_sended_arhs { MEO_RTS, MKEO_RTS, MEO_RTS_NOTE, MEO_MKPI, MEO_MKPI_NOTE, MEO_MKPO, MEO_UN, MEO_LDR };
// enum ids_inf_arr_for_uso {None, id_MKPO_for_USO, id_MEO_NK, id_MEO_LA, id_MEO_RTS, id_MEO_RS, id_MKKKR_for_USO, id_MKLDR_for_USO};

// enum command {ADD_MEO_ARH, RET_MEO_ARH, CREATE_MKEO, SEND_MKEO_BKU, RET_MKEO_ARH, RET_TAB_DATA_MEO, RET_TAB_DATA_MKEO, DEL_MEO_ARH };
...
// enum recording_mode_period {ALL_RECORDS, STANDARD, EXTENDED};

// определим и инициализируем наименование конфигурационного файла
// const char* conf_file_name = "conf.txt";

// определим и инициализируем название ключей в конфигурационном файле приложения
//const char*	dir_meo_key = "dir_meo";
//const char* dir_mkeo_key = "dir_mkeo";
//const char* mod_io_key = "mod_io";
//const char* mod_bcu_key = "mod_bcu";

// определим и инициализируем наименования бинарных файлов, хранящих информацию о всех МЭО или МКЭО
//const char* name_meo_data_files = "data_meo";
//const char* name_mkeo_data_files = "data_mkeo";
//const char* name_mkeo_rts = "mkeo_rts";					// тот МКЭО РТС, который в последний раз был передан в имитатор БКУ, т.е. после каждой выдачи в БКУ
//														// текущий МКЭО РТС должен быть сохранен в файл mkeo_rts
//const char* name_mkeo_mkpo = "mkeo_mkpo";				// тот МКЭО MKPO, который в последний раз был передан в имитатор БКУ, т.е. после каждой выдачи в БКУ
//														// текущий МКЭО MKPO должен быть сохранен в файл mkeo_mkpo
//const char* name_mkeo_ldr = "mkeo_ldr";				// тот МКЭО ЛДР, который в последний раз был передан в имитатор БКУ, т.е. после каждой выдачи в БКУ
//														// текущий МКЭО MKPO должен быть сохранен в файл mkeo_mkpo
//const char* name_meo_mkpo = "meo_mkpo";
//const char* name_meo_ldr = "meo_ldr";
//const char* name_current_mkeo_rts = "current_mkeo_rts"; // такой МКЭО РТС, который изменяется и вновь сохраняется после каждой команды типа ADD_MEO_ARH
//														// и готов к выдаче в имитатор БКУ
//const char* old_meo_rts = "old_meo_rts";
//const char* old_meo_mkpo = "old_meo_mkpo";
//const char* old_meo_ldr = "old_meo_ldr";
//
//const char* name_mkpi_meo_folder = "/mkpi";
//const char* name_rts_meo_folder = "/rts";
//const char* name_mkpo_meo_folder = "/mkpo";
//const char* name_ldr_meo_folder = "/ldr";

// определяем структуру для получения информации
//DataMeoWorker MInfo;
//DataMeoWorker MInfoOld;
//config_data ConfData;

// void validateElementMEORTS(const char* name_file, mkeo_rts_t *ptr_rts);
// int division_with_rounding(float divisible, float divisor);
// void formTimeInterval(unsigned char num_time_interval, unsigned char qn, unsigned char j, unsigned char cmr, float value, float range, WORD *format_elem_meo_rts, int *index_format_elem_meo_rts);
// void packagingElementMEORTS(mkeo_rts_t *el_rts, WORD *format_elem_meo_rts, int *size_format_elem_meo_rts);	// упаковка элемента МЭО РТС в соответствии с принятым форматом



// void createMKEO(MeoWorker* ptrMeoWorker, bool isPriorAlg);									// создает МКЭО одним из двух предложенных алгоритмом
// void saveMEOArh(string name_file);															// сохраняет, полученный МЭО в архив
// int deleteMEOArh(ConfigStorageInfoArjs* ptrConfigStorageInfoArjs, string name_file, int id_meo, int type_meo); // удаляет элемент МЭО из архива
// int extractMEOArh(string name_file);														// выгружает в ОП ДПАО указанный МЭО для передачи в ИО
// int extractOldMEOArh(string name_file);														// выгружает в ОП ДПАО указанный МЭО для создания МКЭО
// void network_MKEO_to_client(int type_meo, InfoArrayWorker *ptrInfArrWorker, const char* name_admin);	// передает МКЭО по сети конкретному клиенту
// void network_MEO_to_client(const char* name_admin);											// передает МЭО по сети конкретному клиенту
// void show_MEO_to_client();																	// формирует ответную структуру для возврата МЭО по сети
double strToDouble(std::string token);						// получает строку, меняет ',' на '.', переводит строку в double
bool StartWinsock();
// int server(AsterixWorker* ptrAsterixWorker);			// переводит Сервис в режим приема сообщений и выполнения расчетных задач

// extern void airports::WgsToLocPr(double dLat, double dLon, double *X, double *Y);
//////////тестовые функции/////////
...
void testTransitionToAnotherCoordinateSystem();			// тестирование перевода из одной системы координат в другую (поиск оптимальных коэффициентов)
void testRefPointAndKoeffMasshtab();							// тестирует поиск географических координат нулевой референсной точки 
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
public:
	...
	double getWgs84_Lon() const  { return wgs_84_point.wgs_X_lon; }									// возвращает долготу в системе Wgs84
	...
	double getWgs84_Lat() const  { return wgs_84_point.wgs_Y_lat; }										// возвращает широту в системе Wgs84
...
public:
	...
	virtual ~Line() { }
	double getIntersectionCPA_Y(double CPA_X) 	{ 
		double F = k * CPA_X + m;
		return F; 		// возвращает значение функции у = кх+m в формате КТА
	}
};
...
private:
	...
	Point* ptrAirRefPoint;								// указатель на контрольную точку аэродрома
...
public:
	// ~operationList();											// удаление всех операций из списка (освобождение ресурсов)
	...
	// void insertOperation(operation* oper);              	  // вставка новой операции в соответствии с номером
	// void replacementOperation(operation* oper);			  // замена операции в соответствии с номером
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
// int init_dpao(const char* conf_file_name, ConfigStorageInfoArjs* ptrConfigStorageInfoArjs);						// чтение конфиг. настроек и инициализация нач. данных
// string getFullNameInfoArjFile(ConfigStorageInfoArjs* ptrConfigStorageInfoArjs, bool take_data_from_archive);	// возвращает полное наименование архива ИМ
// void network_name_data_meo(ConfigStorageInfoArjs* ptrConfigStorageInfoArjs, const char* name_admin);			// передает по сети наименование всех МЭО
// void network_name_data_meo(ConfigStorageInfoArjs* ptrConfigStorageInfoArjs, tab_names_inf_array* tab_meo, int type_meo); // формирует наименование всех МЭО указанного типа
// void network_name_data_mkeo(ConfigStorageInfoArjs* ptrConfigStorageInfoArjs, const char* name_admin);		// передает по сети наименование всех МKЭО
// void network_name_data_mkeo(ConfigStorageInfoArjs* ptrConfigStorageInfoArjs, tab_names_inf_array* tab_mkeo);	// формирует наименование всех МKЭО

// int deleteMEOArh(ConfigStorageInfoArjs* ptrConfigStorageInfoArjs, string name_file, WORD num_meo, int type_meo); // удаляет элемент МЭО из архива

// unsigned int CalcHash(char *str);

///////////////класс userInterface  предназначен для интерактивной работы с пользователем в режиме командной строки//////////////////////
class AsterixWorker
{
private:
	...
	//std::vector<unsigned char> msg_MessageType;				// хранит элементы данных I011/000 [тип сообщения] для поступающих на обработку данных	
	//std::vector<data_source_id> msg_DataSourceId;				// хранит элементы данных I011/010 [идентификатор источника данных] для поступающих на обработку данных
	//std::vector<unsigned char>	msg_ServiceId;						// хранит элементы данных I011/015 [идентификация услуги] для поступающих на обработку данных
	//std::vector<position_wgs84>	msg_Position_wgs84;			// хранит элементы данных I011/041 [положение в координатах wgs84] для поступающих на обработку данных
	//std::vector<position_cartesian>msg_PositionCartesian;		// хранит элементы данных I011/041 [положение в декартовых координатах] для поступающих на обработку данных
	//std::vector<unsigned short> msg_Mode3A;						// хранит элементы данных I011/060 [код режима 3/A в восьмеричном представлении]
	//std::vector<double> msg_FlightLevel;								// хранит элементы данных I011/060 [код режима 3/A в восьмеричном представлении]
	//std::vector<double> msg_GeoAltitude;							// хранит элементы данных I011/092 [вычисленная геометрическая высота трассы]
	//std::vector<baro_altitude>	msg_BaroAltitude;					// хранит элементы данных I011/093 [вычисленная барометрическая высота трассы]
	//std::vector<double> msg_TimeOfTrack;							// хранит элементы данных I011/140 [время трассовой информации]
	//std::vector<unsigned short> msg_TrackNumber;				// хранит элементы данных I011/161 [номер трассы]
	//std::vector<track_status> msg_TrackStatus;						// хранит элементы данных I011/170 [статус трассы]
	//std::vector<track_velocity> msg_TrackVelocity;				// хранит элементы данных I011/202 [вычисленная скорость трассы]
	//std::vector<calc_acceleration> msg_CalcAcceleration;		// хранит элементы данных I011/210 [вычисленная ускорение трассы]
	//std::vector<double> msg_CalcVertRate;							// хранит элементы данных I011/215 [вычисленный темп набора/снижения высоты]
	//std::vector<call_sign> msg_CallSign;								// хранит элементы данных I011/245 [идентификация цели]
	//std::vector<target_size_orient> msg_TargetSizeOrient;		// хранит элементы данных I011/270 [размер и ориентация цели]
	//std::vector<ages> msg_Ages;											// хранит элементы данных I011/290 [возраст обновления системной трассы]
	//std::vector<int> msg_Fleet;											// хранит элементы данных I011/300 [идентификация парка машин]
	//std::vector<prog_msg> msg_Prog;									// хранит элементы данных I011/310 [заранее запрограммированное сообщение]
	//std::vector<TAdsBRelatedData> msg_AdsBRelatedData;	// хранит элементы данных I011/380 [режим S / данные, касающиеся АЗН-В]
		
	void init_sourse_data_cat011();									// выделяет необходимое количество памяти для АСТЕРИКС-данных категории 11
	// void handleCat11(Cat11::Cat11* cat11, std::ofstream &out_csv_file);			// разбирает новую запись категории 11 Астерикс, 
...
public:
	...
	// int writeCSV() const;	// пишет полученное АСТЕРИКС-сообщение 11-й категории в файл с расширением csv 
	...
	void parseAsterixMsgCat11(TRecCategory_11 *ptrCat11);																						// разбирает полученную строку АСТЕРИКС категории 011 [buffAsterixMsgInfo]
