#ifndef __COMMON_LIB_ASTERIX_INCLUDED__
#define __COMMON_LIB_ASTERIX_INCLUDED__
...
/**
 *  @brief Обозначение сообщений о бедствии
 */
typedef enum EmergencyStatus
{

	ES_UNKNOWN               = 0,                      //!< - не установлен
	ES_GENERAL               = 1,                      //!< - общее бедствие
	ES_MEDICAL               = 2,                      //!< - угроза здоровью людей
	ES_MINIMUM_FUEL          = 3,                      //!< - авария по топливу
	ES_NO_COMMUNICATION      = 4,                      //!< - потеря связи
	ES_UNLAWFUL_INTERFERENCE = 5,                      //!< - захват борта (атака)
	ES_DOWNED_AIRCRAFT       = 6,                      //!< - аварийная посадка(снижение по высоте)
	ES_RESERVED              = 7                       //!< -
}EmergencyStatus;

/**
 *  @brief Вид маневрирования сопровождаемого объекта - ускорение
 */
typedef enum TargetModeMovement
{
	TMM_MOVE_STABLE            = 0,                    //!< Вид маневрирования сопровождаемого объекта - равномерное движение
	TMM_TURN_LEFT              = 1,                    //!< Вид маневрирования сопровождаемого объекта - поворот влево
	TMM_TURN_RIGHT             = 2,                    //!< Вид маневрирования сопровождаемого объекта - поворот вправо
	TMM_INCREASING_GROUNDSPEED = 3,                    //!< Вид маневрирования сопровождаемого объекта - набор скорости
	TMM_DECREASING_GROUNDSPEED = 4,                    //!< Вид маневрирования сопровождаемого объекта - снижение скорости
	TMM_VERTICAL_RATE_CLIMB    = 5,                    //!< Вид маневрирования сопровождаемого объекта - набор высоты
	TMM_VERTICAL_RATE_DESCENT  = 6                     //!< Вид маневрирования сопровождаемого объекта - снижение по высоте

}TargetModeMovement;

/**
 *  @brief Выборка по высоте от БРЭО самолета
 */
typedef enum AircraftSelectedAltitude
{
...
}AircraftSelectedAltitude;

/**
 *  @brief Признак источника данных измерения трассы воздушного объекта
 */
typedef enum MeasuredDataSource
{
	MDS_NO_DETECTED          = 0,                      //!< Признак источника данных - не определен
	MDS_PSR                  = 1,                      //!< Признак источника данных - Первичный радиолокатор (ПРЛ)
	MDS_SSR                  = 2,                      //!< Признак источника данных - Вторичный радиолокатор (ВРЛ)
	MDS_PSR_SSR              = 3,                      //!< Признак источника данных - ПРЛ и ВРЛ
	MDS_MODE_S_ALL_CALL      = 4,                      //!< Признак источника данных - приемник Mode S общий опрос
	MDS_MODE_S_ROLL_CALL     = 5,                      //!< Признак источника данных - приемник Mode S "перекличка"
	MDS_MODE_S_ALL_CALL_PSR  = 6,                      //!< Признак источника данных - приемник Mode S общий опрос + ПРЛ
	MDS_MODE_S_ROLL_CALL_PSR = 7                       //!< Признак источника данных - приемник Mode S "перекличка" + ПРЛ
}MeasuredDataSource;


/**
 *  @brief Признак государственной принадлежности
 *         при работе с каналом НРЗ
 */
typedef enum IdentificationFrendFoeRezult
{
	IFFR_ALIEN          = 1,
	IFFR_OWN            = 2,
	IFFR_OWN_GUARANTEED = 3,
	IFFR_OWN_INDIVIDUAL = 4,
	IFFR_NOT_TESTED     = 5
}IdentificationFrendFoeRezult;

/**
 *  @brief Признак состояния(доступности) стоянки
 */
enum InterrogateSystemType
{
	IST_UNKNOWN = 0,
	IST_UVD     = 1,
	IST_NRZ     = 2,
...
typedef enum InterrogateSystemType InterrogateSystemType;
/**
 *  @brief Признак состояния(доступности) стоянки
 */
enum StandStatus
{
	SS_EMPTY    = 0,                                   //!< Признак занятости стоянки - стоянка пуста(свободна)
	SS_OCCUPIED = 1,                                   //!< Признак занятости стоянки - стоянка занята(не свободна)
	SS_UNKNOWN  = 2                                    //!< Признак занятости стоянки - состояние не известно
};
typedef enum StandStatus StandStatus;

/**
 *  @brief Признак привязки времени плана
 */
enum PlanDayAttachment
{
	PDA_TODAY     = 0,                                 //!< Признак действия времени плана - текущие сутки
	PDA_YESTERDAY = 1,                                 //!< Признак действия времени плана - вчера
	PDA_TOMORROW  = 2                                  //!< Признак действия времени плана - завтра
};
typedef enum PlanDayAttachment PlanDayAttachment;

/**
 *  @brief Признак допуска к RVSM
 */
enum RvsmAvailable
{
	RA_UNKNOWN      = 0,                               //!< Признак допуска RVSM - не известен
	RA_APPROVED     = 1,                               //!< Признак допуска RVSM - допущен
	RA_EXTEMPT      = 2,                               //!< Признак допуска RVSM - свободный допуск
	RA_NOT_APPROVED = 3                                //!< Признак допуска RVSM - не допущен
};
typedef enum RvsmAvailable RvsmAvailable;
/**
 *  @brief Правила выполнения полета поле 4(бит 4,5) эд 390
 */
enum FligthRulesType
{
	FRT_INSTRUMENTAL     = 0,                          //!< Правила выполнения полета - полет по приборам
	FRT_VISUAL           = 1,                          //!< Правила выполнения полета - полет с визуальным контролем
	FRT_CONTROLED_VISUAL = 3                           //!< Правила выполнения полета - полет с визуальным контролем
	                                                   //!< и диспетчерской поддержкой
};
typedef enum FligthRulesType FligthRulesType;
/**
 *  @brief Тип времени плана для поля 12 эд 380
 */
enum TimeOfDepartureArrival
{
	TODA_SCHEDULED_OFF_BLOCK   = 0,                    //!< Тип времени - планируемое время схода со стояночных колодок
	TODA_ESTIMATED_OFF_BLOCK   = 1,                    //!< Тип времени - расчетное время схода со стояночных колодок
	TODA_ESTIMATED_TAKE_OFF    = 2,                    //!< Тип времени - расчетное время взлета
	TODA_ACTUAL_OFF_BLOCK      = 3,                    //!< Тип времени - актуальное (действительное) время схода со стояночных колодок
	TODA_PREDICTED_HOLD_RUNWAY = 4,                    //!< Тип времени - предполагаемое время занятия ВПП
	TODA_ACTUAL_HOLD_RUNWAY    = 5,                    //!< Тип времени - актуальное (действительное ) время занятия ВПП
	TODA_ACTUAL_LINE_UP        = 6,                    //!< Тип времени - актуальное (действительное) время занятия исходной позиции на ВПП
	TODA_ACTUAL_TAKE_OFF       = 7,                    //!< Тип времени - актуальное (действительное) время взлета
	TODA_ESTIMATED_ARRIVAL     = 8,                    //!< Тип времени - расчетное время прибытия
	TODA_PREDICTED_LANDING     = 9,                    //!< Тип времени -  предполагаемое время посадки
	TODA_ACTUAL_LANDING        = 10,                   //!< Тип времени - актуальное (действительное) время посадки
	TODA_ACTUAL_OFF_RUNWAY     = 11,                   //!< Тип времени - актуальное (действительное) время схода с ВПП
	TODA_PREDICTED_TO_GATE     = 12,                   //!< Тип времени - предполагаемое время подачи к высадке пассажиров
	TODA_ACTUAL_ON_BLOCK       = 13,                   //!< Тип времени - актуальное (действительное) время постановки на стоянку
	TODA_MAX_COUNT             = 14                    //!< - общее количество типов времен
};
typedef enum TimeOfDepartureArrival TimeOfDepartureArrival;


/**
 * @brief Идентификатор типа категории передатчика
 */
