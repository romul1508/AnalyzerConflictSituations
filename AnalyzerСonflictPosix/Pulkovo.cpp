...
void WgsToLocPlkv_zone_1_square_11(double Lat, double Lon, double *pX, double *pY)
{
	...
	temp_lat = Lat - 59.8;				// remove the uninformative part
	temp_lat = temp_lat / 0.001871;		// normalize the remainder

	//------------------
	blk_1->inputs[0] = (float)temp_lat;
	
	contr_fuzzy( blk_1 );
	*pY = blk_1->outputs[0] * 2000.0;	// the result is returned in meters
	//---------------------
	...
	temp_lon = Lon - 30.3;			// the uninformative part is discarded
	temp_lon = temp_lon * 100.0;	// the number of zeros before the decimal point is reduced

	//------------------
	blk_2->inputs[0] = (float)temp_lon;
	
	contr_fuzzy( blk_2 );
	*pX = blk_2->outputs[0] * 2000.0;		// the result is returned in meters
	//---------------------
...
void PlkvLocToWgs_zone_1_square_11(double X, double Y, double *pLat, double *pLon)
{
	...
	*pLat = temp_y * 0.001871 + 59.8;
	...
	temp_x = X * 0.0005;		// fuzzification X (divide by 2000)
...
void WgsToLocPlkv_zone_1_square_25(double Lat, double Lon, double *pX, double *pY)
{
	...
	temp_lat = Lat - 59.8;					// the uninformative part is removed
	temp_lat = temp_lat / 0.0055995;		// the remainder is normalized
	...
	*pY = blk_1->outputs[0] * 2000.0;		//  the result is returned in meters
	...
	temp_lon = Lon - 30.27;					
	temp_lon = temp_lon * 100.0;		

	//------------------
	blk_2->inputs[0] = (float)temp_lon;
	
	contr_fuzzy( blk_2 );
	*pX = blk_2->outputs[0] * 2000.0;		
	//---------------------
...
void WgsToLocPlkv_zone_1_square_26(double Lat, double Lon, double *pX, double *pY)
{
	...
	temp_lat = Lat - 59.8;					
	temp_lat = temp_lat / 0.005567;			

	//------------------
	blk_1->inputs[0] = (float)temp_lat;
	
	contr_fuzzy( blk_1 );
	*pY = blk_1->outputs[0] * 2000.0;		
	//---------------------
