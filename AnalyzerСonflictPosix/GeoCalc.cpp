...
bool LatLon2UTM::validate(double latitude, double longitude)
{
    ...
    {
        //ern::Log::getInstance()
		//		.Write("Legal ranges: latitude [-90,90], longitude [-180,180).");
...
bool CoordinateUTMConversion::validate(double latitude, double longitude)
{
	...
	if (latitude < -90.0 || latitude > 90.0 || longitude < -180.0 || longitude >= 180.0)
    {
		...
		return false;	// if the initial data is not correct
    }
	else
		return true;
}
...
int GeoCalc::Find_Root(double A, double B, double cosLon, double z, double e2, 
...
{
	// finds the desired root of longitude in the CK-42 coordinate system 
...
int GeoCalc::P3_90_02_SK42(double Lat, double Lon, double H, double *ph_wgs, double *pLatOut, double *pLonOut)
{
	...
	if(abs(X_sk_42) > 0.000001 && (abs(Y_sk_42/X_sk_42) < Pi/2 - 0.000001 
		|| abs(Y_sk_42/X_sk_42) > Pi/2 + 0.000001) )
...
double GeoCalc::SK42_P3_90_02_Lat(double Lat, double Lon, double H)
{
	...
	dB = (ro/(M + H)) * ( (N/a) * e2 * sinLat * cosLat * dA 
		...
		Wx * sinLon * (1 + e2 * cos(2.0 * B)) + Wy * cosLon * (1 + e2 * cos(2.0 * B) ) 
		- ro * m * e2 * sinLat * cosLat;

	dL = (ro/((N + H) * cosLat ) ) * (-dX * sinLon + dY * cosLon) + tan(B) * (1 - e2) 
		* (Wx * cosLon + Wy * sinLon) - Wz;
	...
	dB = (ro/(M + H)) * ( (N/a) * e2 * sinLat * cosLat * dA 
		...
		+ Wy * cosLon * (1 + e2 * cos(2.0 * B) ) - ro * m * e2 * sinLat * cosLat;
...
//-----------------------------------------------------
int GeoCalc::P3_90_02_WGS_84(double Lat, double Lon, double H, double *ph_wgs, 
	double *pLatOut, double *pLonOut)
{
	...
	if(abs(X_wgs_84) > 0.000001 && (abs(Y_wgs_84/X_wgs_84) < Pi/2 - 0.000001 
		|| abs(Y_wgs_84/X_wgs_84) > Pi/2 + 0.000001) )
...
//-----------------------------------------------------
int GeoCalc::P3_90_02_WGS_84(double Lat, double Lon, double H, double dX, double dY, 
	double dZ, double Wx, double Wy, double Wz, double m, double *ph_wgs, double *pLatOut, 
	double *pLonOut)
{
...
//-----------------------------------------------------
int GeoCalc::P3_90_02_P3_90_11_WGS_84(double Lat, double Lon, double H, double *ph_wgs, 
	double *pLatOut, double *pLonOut)
{
	// Conversion of geodetic coordinates from the P3-90.02 system to the WGS-84 system 
	...
	int res;
	double B, L, sinLat, cosLat, cosLon, sinLon, sin2Lat, N_p3_90_02, N_wgs_84;
	double X_p3_90_02, Y_p3_90_02, Z_p3_90_02, X_p3_90_11, Y_p3_90_11, Z_p3_90_11; 
	double	X_wgs_84, Y_wgs_84, Z_wgs_84, h_wgs;
...
{
	// More accurate conversion of geodetic coordinates from the CK-42 system 
	// to the WGS-84 system latitude recalculation	
	double dB, dL, dH, B, L, sinLat, cosLat, cosLon, sinLon, sin2Lat, M, N;
...
{
	// More accurate conversion of geodetic coordinates from the WGS-84 system 
	// to the CK-42 system longitude conversion
